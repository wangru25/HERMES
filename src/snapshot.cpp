...
void Snapshot::buildRipsShape(const std::string& filename){
//    m_rips.readDistanceMatrix(filename);
    m_rips.readPointCloud(filename);
    m_rips.convertToDistanceMatrix();
    m_rips.preprocess();
}
...
void Snapshot::takeSnapshots(){
    ...
    // Thus we can avoid reallocation when dynamically filling the matrices
    // We reserve 2 slots for the column of ED0T because each edge has 2 boundary vertices
    // Similar for others
    SparseMatrix ED0T(numVertices, numEdges);
    SparseMatrix ED1T(numEdges, numFacets);
    SparseMatrix ED2T(numFacets, numCells);
    ED0T.reserve(Eigen::VectorXi::Constant(numEdges, 2));
    ED1T.reserve(Eigen::VectorXi::Constant(numFacets, 3));
    ED2T.reserve(Eigen::VectorXi::Constant(numCells, 4));

    std::cout<<CONSOLE_GREEN<<"Fill boundary operators ... "<<CONSOLE_WHITE<<std::endl;
    fillED0T(ED0T, 0, numEdges);
    fillED1T(ED1T, 0, numFacets);
    fillED2T(ED2T, 0, numCells);

    std::cout<<CONSOLE_GREEN<<"Start Filtration: "<<m_filtration.size()<<CONSOLE_WHITE<<std::endl;
    for(int i=0; i<m_filtration.size(); ++i){
        std::cout<<CONSOLE_GREEN<<"Filtration: "<<i<<CONSOLE_WHITE<<std::endl;

        std::cout<<CONSOLE_YELLOW<<"Determine next size ... "<<CONSOLE_WHITE<<std::endl;
...
//        buildLaplacian2(L2, ED1T, ED2T, next_edge_size, next_facet_size, next_cell_size);
        ...
        buildPersistentLaplacian2(L2, ED1T, ED2T, next_edge_size, next_facet_size,
                                  next_cell_size, m_filtration[i], m_p);

        std::cout<<CONSOLE_GREEN<<"Take snapshots ... "<<CONSOLE_WHITE<<std::endl;
        takeVertexSnapshots(L0, numVertices);
...
void Snapshot::setNumElements(){
    if (m_complex == 'r'){
        numVertices = m_rips.numVertices();
        numEdges = m_rips.numEdges();
        numFacets = m_rips.numFacets();
        numCells = m_rips.numCells();
    }
    else{
        numVertices = m_alpha.numVertices();
        numEdges = m_alpha.numEdges();
        numFacets = m_alpha.numFacets();
        numCells = m_alpha.numCells();
    }
}
...
void Snapshot::determineNextEdgeSize(const int& current_size, int& next_size, double filtration){
    ...
    if (m_complex == 'r'){
        while(next_size < m_rips.m_simple_edges.size() && m_rips.m_simple_edges[next_size].length < filtration){
            ++next_size;
        }
    }
    else {
...
void Snapshot::determineNextFacetSize(const int& current_size, int& next_size, double filtration){
    next_size = current_size;
    if (m_complex == 'r'){
        while(next_size < m_rips.m_simple_facets.size() && m_rips.m_simple_facets[next_size].maxdistance < filtration){
            ++next_size;
        }
    }
    else{
...
void Snapshot::determineNextCellSize(const int& current_size, int& next_size, double filtration){
    next_size = current_size;
    if (m_complex == 'r'){
        while(next_size < m_rips.m_simple_cells.size() && m_rips.m_simple_cells[next_size].maxdistance < filtration){
            ++next_size;
        }
    }
    else{
...
void Snapshot::fillED0T(SparseMatrix& ED0T, const int& current_size, const int& next_size){
    if (m_complex == 'r'){
        for(int i=current_size; i<next_size; ++i){
            ED0T.insert(m_rips.positiveEdgeMap[i], i) = 1;
            ED0T.insert(m_rips.negativeEdgeMap[i], i) = -1;
        }
    }
    else {
        for(int i=current_size; i<next_size; ++i){
...
void Snapshot::fillED1T(SparseMatrix& ED1T, const int& current_size, const int& next_size){
    if (m_complex == 'r'){
        for(int i=current_size; i<next_size; ++i){
            ED1T.insert(m_rips.positiveFacetMap[i].first, i) = 1.;
            ED1T.insert(m_rips.positiveFacetMap[i].second, i) = 1.;
            ED1T.insert(m_rips.negativeFacetMap[i], i) = -1.;
        }
...
void Snapshot::fillED2T(SparseMatrix& ED2T, const int& current_size, const int& next_size){
    if (m_complex == 'r'){
        for(int i=current_size; i<next_size; ++i){
            ED2T.insert(m_rips.positiveCellMap[i].first, i) = 1.;
            ED2T.insert(m_rips.positiveCellMap[i].second, i) = 1.;
            ED2T.insert(m_rips.negativeCellMap[i].first, i) = -1.;
            ED2T.insert(m_rips.negativeCellMap[i].second, i) = -1.;
        }
    }
    else{
        for(int i=current_size; i<next_size; ++i){
...
void Snapshot::buildPersistentLaplacian0(SparseMatrix& L0, const SparseMatrix& ED0T, const SparseMatrix& ED1T, const int& edge_size, const double filtration, double p) {
        ...
        determineNextEdgeSize(edge_size, next_size, filtration + p);
...
void Snapshot::buildPersistentLaplacian1(SparseMatrix& L1, const SparseMatrix& ED0T, const SparseMatrix& ED1T, const int& edge_size, const int& facet_size, const double filtration, double p) {
    ...
    determineNextFacetSize(facet_size, next_size, filtration + p);
...
void Snapshot::buildPersistentLaplacian2(SparseMatrix& L2, const SparseMatrix& ED1T, const SparseMatrix& ED2T, const int& edge_size, const int& facet_size, const int & cell_size, const double filtration, double p) {
	...
	next_size = cell_size;
    determineNextCellSize(cell_size, next_size, filtration + p);
...
void Snapshot::takeVertexSnapshots(const SparseMatrix& L0, const int matrix_size){
    ...
    matlabEIGS(eigenvalues, eigenvectors, numVertices, row, col, val, m_num_eigenvalues);
    auto time_end = Clock::now();
    auto secs = std::chrono::duration_cast<TimeSecond>(time_end-time_start);
    std::cout<<"Elapsed time: "<<secs.count()<<std::endl;

    if(eigenvalues.empty()){
        int n = std::min(numVertices, m_num_eigenvalues);
